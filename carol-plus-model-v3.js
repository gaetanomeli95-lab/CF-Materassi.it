...
import {roundedSlabGeometry,createCarolMaterials,frame,ribbon,handles,topPattern} from './carol-plus-materials-v3.js?v=carol-real-photo-v2';

// Centimetres converted to scene units. The assembled silhouette follows a
// 160 x 190 cm mattress, approximately H27.
const W=160/30,D=190/30;

function helix(){
  const pts=[],turns=5.4,segments=88;
  for(let i=0;i<=segments;i++){
    const t=i/segments,a=t*Math.PI*2*turns;
    const r=.034 + Math.sin(t*Math.PI)*.004;
    pts.push(new THREE.Vector3(Math.cos(a)*r,(t-.5)*.37,Math.sin(a)*r));
  }
  return new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts),88,.0043,5,false);
}

function springCore(){
  const group=new THREE.Group(),cols=40,rows=40,frontRows=5,rearRows=rows-frontRows;
  const radius=.050,xSpan=W-.82,zSpan=D-.82;
  const pocketGeo=new THREE.CylinderGeometry(radius,radius,.40,12,1,false),springGeo=helix();

  // 1600 individually pocketed micro-springs: warm white textile sleeves,
  // with the front rows translucent enough to reveal the real steel coil.
  const rearPocketMat=new THREE.MeshPhysicalMaterial({color:0xf1eee8,roughness:.98,sheen:.12});
  const frontPocketMat=new THREE.MeshPhysicalMaterial({color:0xf7f4ef,roughness:.96,transparent:true,opacity:.46,depthWrite:false,side:THREE.DoubleSide});
  const rearSpringMat=new THREE.MeshStandardMaterial({color:0xc8c7c2,metalness:.52,roughness:.48});
  const frontSpringMat=new THREE.MeshStandardMaterial({color:0xd7d5cf,metalness:.68,roughness:.34});
  ...
  // MK5 anti-sag perimeter box, left open in the central front area so the
  // pocketed core is clearly visible during the exploded sequence.
  const boxMat=new THREE.MeshPhysicalMaterial({color:0xe7dfd2,roughness:.96});
  ...
  const frontL=new THREE.Mesh(new THREE.BoxGeometry(.74,.40,.24),boxMat);frontL.position.set(-W/2+.47,0,D/2-.22);
  const frontR=frontL.clone();frontR.position.x=W/2-.47;
...
export function buildCarolPlusModel({mobile=false}={}){
  ...
  // Technical stack taken from the CF Carol Plus sheet:
  // cover + lining + PU 3cm + felt + 1600 micro-springs + felt + PU 3cm + memory 5cm + lining + fibre cover.
  ...
  ribbon(lowerShell,W,D,.13,mats.tex.ribbon);
  handles(lowerShell,W,D,-.15);
